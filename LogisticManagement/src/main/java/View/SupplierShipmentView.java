...
public class SupplierShipmentView {

    public static JPanel createShipmentPanel() {
    	
    	List<Order> orders = OrderController.getOrderByStatus(orderStatus.KARGODA);
        ...
        for (Order o : orders) {
            ...
            JButton deliveredBtn = createButton("Teslim Edildi");
            JButton failedBtn = createButton("Teslim Edilemedi");
            ...
            deliveredBtn.addActionListener(e -> {
                o.setStatus(orderStatus.TESLIM_EDILDI);
                JOptionPane.showMessageDialog(null,
                        "Sipariş teslim edildi olarak güncellendi (Order #" + o.getId() + ")");
                OrderController.updateOrder(o);
                ViewController.RefreshContentPanel(Route.SHIPMENT_SP);
            });

            failedBtn.addActionListener(e -> {
                o.setStatus(orderStatus.TESLIM_EDILEMEDI);
                JOptionPane.showMessageDialog(null,
                        "Sipariş teslim edilemedi olarak güncellendi (Order #" + o.getId() + ")");
                OrderController.updateOrder(o);
                ViewController.RefreshContentPanel(Route.SHIPMENT_SP);
            });
